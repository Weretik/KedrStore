...
`SyncOneCOrdersJob` is registered in Sales Infrastructure and invoked by the existing one-run `Host.Jobs` host. It claims due records safely, including stale `Sent` attempts, obeys the Kyiv delivery window, sends the configured batch with pacing, records outcomes, and applies the retry schedule. `OneCOrderSyncOptions` is owned and validated by Sales Infrastructure and binds the `Sales:OneCOrderSync` section; it controls batch size, inter-call delay, maximum attempts, sending-window bounds, and stale-`Sent` recovery delay. Stale recovery reuses the stable `OrderNumber`, which OneC treats as duplicate-safe. Structured logs include `OrderId`, `AttemptCount`, `Status`, and correlation context; metrics cover accepted, transport/business errors, retry queue, and dead-letter queue.

When a delivery enters `DeadLetter`, a Sales Infrastructure Telegram adapter sends one failure notification. Notification attempts are durable on `OneCOrderSync`, claimed with PostgreSQL `xmin`, and retried at `5m -> 15m -> 30m -> 1h -> 3h` for at most five attempts. After exhaustion, the failure remains observable and the `DeadLetter` state is unchanged. A successful notification sets `DeadLetterNotifiedAtUtc`, preventing another send. Its message identifies the customer, order, and final sanitized error; it attaches an Excel file with the persisted line snapshots. The notifier and Excel exporter use Sales Application abstractions and must not couple Sales to Catalog's public quick-order request types.

Manual retry is an Application command backed by `IOrderSyncRetryStore`. Its Infrastructure adapter loads the order and synchronization record, verifies the terminal status, appends `OrderSyncRetryAudit`, and changes the aggregate to `RetryScheduled` in one `SaveChanges` transaction. The aggregate resets per-cycle attempt and notification state while the audit retains the previous failure. The API returns after persistence, and the existing delivery job performs the later SOAP call. The same `xmin` token rejects concurrent retry transitions.
